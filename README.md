...
母文档：[`proposal.html`](proposal.html)（方案）、[`survey.html`](survey.html)（背景综述）。
...
data/docs/docs_clean.jsonl   多体裁讨论文档，过完泄漏/去重质量闸
   ...
   ▼
data/corpus/{train,val}.bin  →  checkpoints/   （打包 + midtrain Qwen3.5-4B-Base）
```
...
# 依赖：torch, transformers, numpy, pyyaml, tqdm, requests（合成走 OpenRouter HTTP）
```
...
```
configs/        sources.yaml（采集源）, train_qwen3p5_4b.yaml（8卡）, train_smoke.yaml（冒烟）
prompts/        skill2spec_v1.md, doc_ideas_v1..v4.md, doc_writer_v1..v4.md, doc_genres.yaml, DEBUG_LOG.md
scripts/        00_env.sh ~ 05_train.sh（薄封装，透传参数）
...
- ⬜ 8 卡正式 midtrain 未启动；Stage-2 tool-use SFT 另行。

...
- **[SSM]** 实际实现是**自洽的 inspect-free 跑分器** `src/exp/run_eval.py`（`scripts/10_eval.sh`），与本仓库 `client.py` 栈一致、可直接吃自己的 checkpoint：
  E1/E2/E3/E4 用可判定标准答案直接算（exact-match 工具名 / 调用-vs-拒答），E5/E6 用 LLM-judge（`OpenRouterClient`，judge 与被测分离，`--judge-model` 可换强模型），
  `--epochs` 多次重采样求 mean±stderr，`--in-context` 即臂 B1。若要严格对齐 [MSM] 原文，可再包一层 inspect-ai 适配（评分口径相同）。
